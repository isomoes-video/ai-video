...
assert len(list((w/'audio').glob('*.mp3')))==8
assert len(list((w/'audio').glob('*.srt')))==8
pdfinfo=subprocess.check_output(['pdfinfo',str(w/'output.pdf')],text=True)
assert re.search(r'Pages:\s+8\b',pdfinfo)
for e in entries:
 ...
 assert float(v['duration'])+1e-5>=audio+(0.25 if n<8 else 0),(n,'short visual')
